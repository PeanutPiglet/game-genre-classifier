...
Nevertheless, we observe significantly faster training times with PyTorch models, expected since they use optimized tensors for ML and runs on CUDA cores. All models appear to be stuck at around 90% accuracy. Particularly, models seem to converge well towards their optimums with just one epoch. One outlier is the heavy PyTorch dense network, which had a much lower accuracy than the others after many epochs.

To examine whether the models were stuck because of over-fitting, we ran additional tests on the highest-epoch saves of each model, marked with an asterisk. Specifically, we sampled from the training data set and hypothesized a higher accuracy if over-fitting is indeed happening. However, no clear differences were observed. Now, a plausible explanation would be that the models are all simply stuck in a local optimum in the optimization landscape. Future training could use different optimizers and hyper-parameters to escape the potential locality. 
